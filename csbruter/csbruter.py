...
parser.add_argument(
    "-t", dest="threads", default=25, type=int, help="Concurrency level"
)
...
def pwd_chk(password):
    if len(password) > 0:
        ...
        payload = (
            bytearray(b"\x00\x00\xbe\xef")  # Magic number
            + len(password).to_bytes(1, "big", signed=True)
            + bytes(bytes(password, "ascii").ljust(256, b"A"))
        )
        conn.send(payload)
...
if len(passwords) > 0:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.threads) as executor:
        ...
        future_to_check = {
            executor.submit(pwd_chk, password): password for password in passwords
        }
    ...
    print(
        "Attempts per second: {:.1f}".format((failures + attempts) / (finish - start))
    )
else:
